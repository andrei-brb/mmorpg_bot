...
async def _settle_one_auction(db, listing_id: UUID) -> bool:
    ...
    listing = await db.fetchrow(
        """SELECT ml.*, t.name, i.template_id,
           ...
           WHERE ml.id = $1 AND ml.is_active = TRUE
             ...
             AND ml.current_bidder_id IS NOT NULL
             AND ml.current_bid IS NOT NULL""",
        listing_id,
    )
    if not listing:
        return False
    ...
    if not add_ok:
        await char_svc.deduct_gold(listing["seller_id"], pay_amount, "revert: auction settlement transfer failed")
