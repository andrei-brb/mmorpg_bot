...
export function CraftingTab() {
  // All forge state/actions now live in useForge() so the mobile Workbench can
  // drive the same forge. This view renders exactly what it rendered before.
  const {
    char, items, inventory,
    craftLevel, craftXp, needXp, xpPct,
    job, jobTitle, jobStatusLine, secondsLeft, canClaim, successPct, outcome,
    bagGear, selectedId, setSelectedId, selectedItem, sourceRarity,
    forgeMode, setForgeMode, branchRecipeId, setBranchRecipeId,
    forgeOptions, pathA, pathB, rule, branchRecipes, selectedBranch,
    rarityTabDisabled, upgradeTabDisabled,
    playerGold, canAffordRarityPath, canAffordUpgradePath,
    meetsCraftingLevelRarity, meetsCraftingLevelUpgrade,
    startForge, claimForge,
    repairQuote, repairLoading, repairError, repairing, repairAll, loadRepairQuote,
  } = useForge();
