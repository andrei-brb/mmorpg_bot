...
- [x] **Activity overworld combat** — lore gates apply only for `LORE_BOSS_GATES` story bosses; normal mobs and dungeons stay ungated.
...
- [x] **`LORE_BOSS_GATES`** in `config/lore_gates.py` — story bosses with `required_flags` / hints (extend as arc grows).
